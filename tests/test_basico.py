...
def test_abrevadero_coordenadas():
    ...
    # Posición 1 debe estar en el norte (con RADIO = 9.5)
    x, y = abrevadero.obtener_coordenadas(1)
    assert abs(x) < 0.01 and abs(y - 9.5) < 0.01  # Norte
    
    # Posición 5 debe estar en el sur
    x, y = abrevadero.obtener_coordenadas(5)
    assert abs(x) < 0.01 and abs(y + 9.5) < 0.01  # Sur
...
def test_abrevadero_distancia():
    ...
    distancia = abrevadero.calcular_distancia((0, 9.5), (0, 0))
    assert abs(distancia - 9.5) < 0.01
    
    # Test distancia león-impala desde posición 1
    distancia_leon = abrevadero.distancia_leon_impala(1)
    assert abs(distancia_leon - 9.5) < 0.01
...
def test_base_conocimientos():
    ...
    # Usar distancia inicial con nuevo RADIO = 9.5
    estado = Estado(1, 9.5, "ver_frente", False, True)
...
def test_recompensas():
    ...
    # Recompensa por acercamiento (con nuevo RADIO)
    r = sr.calcular_recompensa_acercamiento(9.5, 8.5)
    assert r > 0
